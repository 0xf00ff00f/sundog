...
void main() {
    ...
    // point on the curve
    vec2 current = vec2(semiMajorAxis * cos(meanAnomaly) - focus, semiMinorAxis * sin(meanAnomaly));

    // direction (tangent vector) of the curve
    vec2 direction = vec2(-semiMajorAxis * sin(meanAnomaly), semiMinorAxis * cos(meanAnomaly));
